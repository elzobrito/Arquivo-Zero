...
## Rastreabilidade de funções do jogo

O mapa canônico do motor está em `docs/architecture/function-map.json`; a visão
humana equivalente está em `docs/architecture/function-map.md`. Toda tarefa ESAA
que altere `dist/app.js` ou algum contrato consumido por ele em `dist/game.json`,
`dist/index.html` ou `dist/styles.css` deve:

1. declarar nas notas e nas evidências de `complete` os IDs `FN-*` e `CB-*`
   impactados, incluindo impactos indiretos;
2. atualizar os dois mapas quando assinatura, responsabilidade, chamadas, estado,
   DOM, dados, persistência, eventos ou efeitos colaterais mudarem;
3. executar `node scripts/verify-function-map.mjs` e registrar o resultado;
4. executar também `node --check dist/app.js`, validar `dist/game.json` e realizar
   smoke HTTP quando a alteração puder afetar carregamento ou integração.

Uma tarefa que crie função ou callback deve criar um novo ID estável. A remoção
deve ser registrada no histórico ESAA da tarefa; IDs antigos não são reutilizados.
O mapa não concede autoridade de escrita: os limites e transições ESAA continuam
sendo obrigatórios.
