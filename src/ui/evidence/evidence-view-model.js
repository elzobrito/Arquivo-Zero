...
/**
 * View-model for one discovered evidence record.
 * Labels stay descriptive; never promote unknown → false / inadmissible.
 */

function labelOrUnknown(value, unknownLabel) {
...
function admissibilityLabel(value) {
...
function buildViewModel(record, ctx) {
  const policy = ctx.policy;
  const factText = labelOrUnknown(record.text || record.fact, "Desconhecido");
  const interpretation = record.interpretation != null && record.interpretation !== ""
...
function buildViewModels(records, ctx) {
  const list = Array.isArray(records) ? records : [];
  return list.map(function (record) {
    return buildViewModel(record, ctx || {});
  });
}
