...
/**
 * Renders detail panel markup for one evidence view-model.
 * Fact and interpretation stay in separate regions.
 */
...
function listBlock(title, items, emptyText) {
  if (!items || !items.length) {
    return (
      ...
      '<p class="ev-muted">' + escapeHtml(emptyText) + "</p>" +
      "</section>"
    );
  }
  return (
...
function renderEvidenceDetails(model) {
  if (!model) {
    return (
      '<div class="ev-detail-empty" role="status">' +
      "<strong>Selecione uma evidência</strong>" +
      "<p>O detalhe mostra fato, interpretação (se houver), origem e status da policy — sem concluir autoria.</p>" +
      "</div>"
    );
  }
  ...
  return (
    '<article class="ev-detail" aria-labelledby="ev-detail-title">' +
    '<header class="ev-detail-head">' +
    '<p class="ev-kicker">' + escapeHtml(model.id) +
    (model.essential ? ' · essencial ao mandado' : "") +
    "</p>" +
    '<h3 id="ev-detail-title">' + escapeHtml(model.title) + "</h3>" +
    '<p class="ev-tags"><span>' + escapeHtml(model.tag) + "</span>" +
    "<span>" + escapeHtml(model.admissibilityLabel) + "</span>" +
    "<span>" + escapeHtml(model.integrityLabel) + "</span>" +
    "<span>" + escapeHtml(support) + "</span></p>" +
    "</header>" +
    '<section class="ev-section ev-fact" aria-label="Fato">' +
    "<h4>Fato</h4>" +
    "<p>" + escapeHtml(model.fact) + "</p>" +
    "</section>" +
    ...
    (model.interpretation
      ? ""
      : '<p class="ev-muted">Nenhuma interpretação cadastrada — permanece Desconhecido.</p>') +
    "</section>" +
